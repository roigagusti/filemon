...
rt = Rompetechos(api_key,api_token)
...
class Component:
    def __init__(self,case,project,code,name,feature,blind_cost,case_cost,rat_cost,status='Development'):
        self.case = case
...
def convert_json(json_data):
    ...
    for recordbase in records:
        ...
        component = Component(case_type,project,code,name,feature,blind_cost,case_cost,rat_cost,status)

        if case_type in result:
            if project in result[case_type]:
                result[case_type][project].append(component)
                print(component.case + ':' + component.project + "_" + component.code)
            else:
